...
export default function TouristDetail() {
    ...
    async function fetchDetail() {
      try {
        if (!passedDetails) {
          ...
          const fetchedDetails = Array.isArray(response.data) ? response.data : [response.data];
          setDetails(fetchedDetails);
          setSelectedDetailId(fetchedDetails[0]?.id || null);
  ...
  if (!details || details.length === 0 || !selectedDetailId) return <p>Loading...</p>;

  // ตรวจสอบ selectedDetail และ fallback ค่า default
  ...
  // รวมภาพหลักและภาพเพิ่มเติม
  const allImages = [
    ...(selectedDetail.image
      ? [selectedDetail.image.startsWith("http") ? selectedDetail.image : "/uploads/" + selectedDetail.image]
      : []),
    ...(selectedDetail.images ? selectedDetail.images.map((img) => "/uploads/" + img.image_path) : []),
  ];
  ...
  return (
    <div className="flex flex-col h-screen w-screen bg-white text-slate-900 px-4 md:px-12 mt-12">
      ...
      <div className="w-full h-[300px] bg-white flex flex-col justify-center px-6 md:px-12 py-5 relative overflow-hidden">
        ...
        <h1 className="text-6xl font-extrabold text-slate-900 mb-4 mt-5">รายละเอียดสถานที่</h1>
        <p className="text-2xl text-slate-700 mb-6">รายละเอียดเพิ่มเติมของสถานที่</p>
      </div>
          ...
          {details.map((detail) => (
            <button
              ...
              className={`px-4 py-2 rounded-xl font-semibold ${
                selectedDetailId === detail.id ? "bg-blue-600 text-white" : "bg-blue-100 text-blue-800"
              }`}
            >
              {detail.description ? (detail.description.length > 20 ? detail.description.slice(0, 20) + "..." : detail.description) : "-"}
            </button>
          ))}
        </div>
